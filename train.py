...
utils.create_exp_dir(args.save, scripts_to_save=glob.glob('*.py') + ["model/model.py"] + glob.glob("utils/*.py"))
...
def main():
    ...
    if args.resume:
        try:
            checkpoint = torch.load(args.model_pretrain)
            model.load_state_dict(checkpoint['model_state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            start_epoch = checkpoint['epoch'] + 1
            print(f"Resume training from the checkpoint, starting from epoch {start_epoch}.")
        ...
        try:
            ...
            if args.model_pretrain.endswith(".pth"):
    ...
    for epoch in range(args.epochs):
        ...
        utils.save_all(epoch, model, optimizer, losses[-1], os.path.join(model_path, 'weights_%d.pth' % epoch))
        torch.cuda.empty_cache()

        if epoch % 1 == 0 and total_step != 0:
            model.eval()
            model.is_new_seq = True
            with torch.no_grad():
                for idx, (input, img_name, img_path, last_img_path) in enumerate(test_queue):
                    ...
                    mask = save_images(mask)
                    os.makedirs(args.save + '/result/denoise/', exist_ok=True)
                    os.makedirs(args.save + '/result/enhance/', exist_ok=True)
                    os.makedirs(args.save + '/result/warp/', exist_ok=True)
                    os.makedirs(args.save + '/result/mask/', exist_ok=True)
                    ...
                    Image.fromarray(last_H3_wp).save(args.save + '/result/warp/' +input_name+'_last_H3_wp_'+str(epoch)+'.png', 'PNG')
                    Image.fromarray(mask).save(args.save + '/result/mask/' +input_name+'_mask_'+str(epoch)+'.png', 'PNG')
        
                    model.update_H3(H3, s3)
